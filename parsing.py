...
class Parser:
  ...
  def FASES_EPIC(self):
    return self.backtracking([self.EXPLORE, self.PRESENT, self.INTERACT, self.CRITIQUE])
  ...
  def PRESENT(self):
    return self.backtracking([self.VISUALIZAR_PDF, self.VISUALIZAR_VIDEO, self.VIDEOCONFERENCIA]) and self.TEMPO()

  def INTERACT(self):
    return self.backtracking([self.WHATSAPP_WEB, self.EMAIL, self.VIDEOCONFERENCIA]) and self.TEMPO()
